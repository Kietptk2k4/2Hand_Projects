...
export function CommerceWriteReviewScreen() {
  useThemeColors();
  const styles = useThemedStyles(createStyles);
  const { showToast } = useSocialToast();
  const { orderItemId: rawOrderItemId } = useLocalSearchParams();
  const orderItemId = resolveParam(rawOrderItemId);
  ...
  const {
    ...
    errorMessage,
    retry,
  } = useReviewFormPage({ mode: "create", orderItemId });
  ...
  const handleSubmit = useCallback(
    async ({ payload, pendingFiles }) => {
      setApiError("");
      const targetOrderItemId = resolvedOrderItemId || orderItemId;
    ...
    [navigateAfterSuccess, orderItemId, resolvedOrderItemId, showToast, submitCreate, upload]
  );
