...
function createStyles(colors) {
  return {
    ...
    rawStatus: { fontSize: 13, color: colors.onSurfaceVariant, marginTop: 2, opacity: 0.85 },
    eventTime: { fontSize: 13, color: colors.onSurfaceVariant, marginTop: 2 },
  };
}

export function ShipmentTrackingTimeline({ events, isLoading }) {
  const colors = useThemeColors();
  const styles = useThemedStyles(createStyles);

  return (
    <View style={styles.section}>
      <Text style={styles.title}>Tiến trình vận chuyển</Text>
            ...
            {events.map((event) => (
              <View key={event.id} style={styles.eventRow}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {event.rawStatus ? (
                    <Text style={styles.rawStatus}>{event.rawStatus}</Text>
                  ) : null}
