import {
  ...
  SHOP_MODERATION_PAGE_SIZE_OPTIONS,
...
import { ShopModerationBulkActionBar } from "./ShopModerationBulkActionBar.jsx";
import { ShopModerationFilterBar } from "./ShopModerationFilterBar.jsx";
import { ShopModerationStatsBar } from "./ShopModerationStatsBar.jsx";
import { ShopModerationTable } from "./ShopModerationTable.jsx";

function sortColumnLabel(sortField) {
  const option = SHOP_MODERATION_LIST_SORT_OPTIONS.find((item) => item.value === sortField);
...
function PageSizeSelect({ value, onChange, disabled }) {
  return (
    <label className="inline-flex min-h-11 items-center gap-2 text-sm text-admin-text-secondary">
      ...
      >
        {SHOP_MODERATION_PAGE_SIZE_OPTIONS.map((option) => (
...
export function ShopModerationListView({
  status,
  errorMessage,
  appliedFilters,
  draftFilters,
  ...
  const summary =
    status === "ready"
      ? `${pagination?.total_items ?? 0} cửa hàng · Sắp xếp: ${sortColumnLabel(activeSort)} · Trang ${pagination?.page ?? currentPage}/${totalPages}`
      : "";
  ...
  return (
    <div className="mb-6 max-w-full min-w-0 space-y-6">
      <AdminPageHeader
        ...
        subtitle="Lọc danh sách, mở drawer để xem ngữ cảnh shop và tạm ngưng, đóng hoặc khôi phục."
      />

      <ShopModerationStatsBar stats={stats} status={statsStatus} onPresetClick={onStatPresetClick} />
      ...
      <AdminSurfaceCard padding="none" className="overflow-hidden">
        ...
        {status === "ready" ? (
          <div className="p-4 lg:p-5">
            <ShopModerationBulkActionBar
              selectedCount={selectedShopIds?.length || 0}
              canSuspendShop={canSuspendShop}
              canReopenShop={canReopenShop}
              disabled={bulkSubmitting}
              onSuspend={onOpenBulkSuspend}
