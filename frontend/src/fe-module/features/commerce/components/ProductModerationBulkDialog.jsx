...
import { REASON_MAX_LENGTH } from "../constants/adminProductRemovalConstants.js";

export function ProductModerationBulkDialog({
  open,
  mode,
  selectedCount,
  isSubmitting,
  ...
  const title = mode === "restore" ? "Khôi phục hàng loạt" : "Gỡ listing hàng loạt";
    ...
    >
      <div className="flex max-h-[90dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-2xl border border-admin-border bg-admin-surface shadow-[var(--shadow-admin-surface)] sm:rounded-xl">
        <div className="border-b border-admin-border-subtle px-4 py-4 sm:px-6">
          <h2 id="product-bulk-title" className="text-lg font-semibold text-admin-text">
            {title}
          </h2>
