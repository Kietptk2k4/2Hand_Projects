...
import { getReviewModerationActionLabel } from "../constants/reviewModerationDisplayLabels.js";
import { useReviewModerationHistory } from "../hooks/useReviewModerationHistory.js";

function historyBadgeVariant(action) {
  const normalized = String(action || "").toUpperCase();
  if (normalized.includes("RESTORE")) return "success";
  if (normalized.includes("REMOVE")) return "danger";
  return "neutral";
}

export function ReviewModerationHistoryPanel({ reviewId, refreshToken }) {
  const { canReadReviewHistory } = useContentModerationPermissions();
  const { result, status, errorMessage, refetch } = useReviewModerationHistory(
    canReadReviewHistory ? reviewId : null,
  );
  ...
  useEffect(() => {
    if (!refreshToken || !canReadReviewHistory) return;
    refetch();
  }, [canReadReviewHistory, refreshToken, refetch]);

  if (!canReadReviewHistory || !reviewId) return null;
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <AdminStatusBadge variant={historyBadgeVariant(entry.action)}>
                    {getReviewModerationActionLabel(entry.action)}
                  </AdminStatusBadge>
