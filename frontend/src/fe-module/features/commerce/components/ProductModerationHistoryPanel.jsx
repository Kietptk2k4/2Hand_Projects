...
import { getProductModerationActionLabel } from "../constants/productModerationDisplayLabels.js";
...
export function ProductModerationHistoryPanel({ productId, refreshToken }) {
  const { canReadProductHistory } = useContentModerationPermissions();
  const { result, status, errorMessage, refetch } = useProductModerationHistory(
    canReadProductHistory ? productId : null,
  );
  ...
  useEffect(() => {
    if (!refreshToken || !canReadProductHistory) return;
    refetch();
  }, [canReadProductHistory, refreshToken, refetch]);

  if (!canReadProductHistory || !productId) return null;
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <AdminStatusBadge variant={historyBadgeVariant(entry.action)}>
                    {getProductModerationActionLabel(entry.action)}
                  </AdminStatusBadge>
