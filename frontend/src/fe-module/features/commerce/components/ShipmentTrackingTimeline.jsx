...
export function ShipmentTrackingTimeline({ events, isLoading }) {
  return (
    <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-4 shadow-sm md:p-6">
      <h2 className="mb-4 text-headline-sm font-semibold text-on-surface">Tiến trình vận chuyển</h2>
            ...
            {events.map((event) => (
              <li key={event.id} className="relative z-10 flex gap-4">
                ...
                <div>
                  ...
                  {event.rawStatus ? (
                    <p className="text-body-sm text-on-surface-variant/80">{event.rawStatus}</p>
                  ) : null}
