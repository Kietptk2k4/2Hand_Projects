...
import {
  mapActiveCategoriesResponse,
  toHomeNavItems,
  toSellerCategoryOptions,
  toSidebarCategoryItems,
} from "../utils/categoriesMapper";
...
export function useCommerceCategories(options = {}) {
  ...
  const sidebarItems = useMemo(() => toSidebarCategoryItems(categories), [categories]);
  const homeNavItems = useMemo(() => toHomeNavItems(categories), [categories]);
  const sellerOptions = useMemo(() => toSellerCategoryOptions(categories), [categories]);
