...
export async function createPostComment(postId, { contentText, media = [] }) {
  try {
    const response = await socialApiClient.post(`/api/v1/social/posts/${postId}/comments`, {
      contentText,
      media,
    });
...
export async function createCommentReply(commentId, { contentText, media = [] }) {
  try {
    const response = await socialApiClient.post(`/api/v1/social/comments/${commentId}/replies`, {
      contentText,
      media,
    });
    return unwrapCreatedResponse(response);
