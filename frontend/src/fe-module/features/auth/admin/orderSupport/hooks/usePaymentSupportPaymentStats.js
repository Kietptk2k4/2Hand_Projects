...
import { PAYMENT_STAT_PRESETS } from "../constants/paymentSupportListConstants.js";

function extractTotal(data) {
  return data?.total_elements ?? data?.totalElements ?? 0;
}

export function usePaymentSupportPaymentStats({ enabled = true } = {}) {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      const presetRequests = PAYMENT_STAT_PRESETS.map((preset) =>
        getPaymentsForSupport({
          page: 1,
          size: 1,
      ...
      const [totalData, ...presetResults] = await Promise.all([
        getPaymentsForSupport({ page: 1, size: 1 }),
        ...presetRequests,
      ]);
