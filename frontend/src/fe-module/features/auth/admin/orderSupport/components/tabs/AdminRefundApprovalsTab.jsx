            ...
            >
              {STATUS_OPTIONS.map((value) => (
                <option key={value || "all"} value={value}>
                  {value ? REFUND_STATUS_LABELS[value] || value : "Tất cả"}
                </option>
              ))}
            </select>
          </label>
                ...
                {queue.items.length ? (
                  queue.items.map((item) => (
                    <tr key={item.id} className="border-b border-outline-variant/50">
                      ...
                      <td className="px-3 py-2">{REFUND_REQUESTED_BY_LABELS[item.requestedBy] || item.requestedBy}</td>
                      <td className="px-3 py-2 font-medium">{formatVndPrice(item.amount)}</td>
                      <td className="px-3 py-2">{item.paymentMethod || "—"}</td>
                      <td className="px-3 py-2">{REFUND_STATUS_LABELS[item.status] || item.status}</td>
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-2">
                          <button
                            type="button"
                            onClick={() => setSelectedRefundId(item.id)}
                            className="rounded border border-outline-variant px-2 py-1 text-label-sm hover:bg-surface-container-high"
                          >
                            Chi tiết
                          </button>
