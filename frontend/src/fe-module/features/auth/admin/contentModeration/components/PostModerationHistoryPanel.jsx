...
import { getPostModerationActionLabel } from "../constants/postModerationDisplayLabels.js";
import { useContentModerationPermissions } from "../hooks/useContentModerationPermissions.js";
import { usePostModerationHistory } from "../hooks/usePostModerationHistory.js";
...
export function PostModerationHistoryPanel({ postId, refreshToken }) {
  const { canReadPostHistory } = useContentModerationPermissions();
  const { result, status, errorMessage, refetch } = usePostModerationHistory(
    canReadPostHistory ? postId : null,
  );

  useEffect(() => {
    if (!refreshToken || !canReadPostHistory) return;
    refetch();
  }, [canReadPostHistory, refreshToken, refetch]);

  if (!canReadPostHistory || !postId) return null;
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <AdminStatusBadge variant={historyBadgeVariant(entry.action)}>
                  {getPostModerationActionLabel(entry.action)}
                </AdminStatusBadge>
              ...
              <p className="mt-2 font-mono text-[11px] text-admin-text-muted">
                Admin: {entry.adminId || "—"}
              </p>
            </li>
          ))}
