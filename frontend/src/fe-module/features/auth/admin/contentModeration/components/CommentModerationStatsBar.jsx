export function CommentModerationStatsBar({ stats, status, onPresetClick }) {
  if (status === "loading") {
    return (
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[1, 2, 3, 4].map((key) => (
          <div key={key} className="h-20 animate-pulse rounded-xl border border-admin-border bg-admin-surface-muted" />
        ))}
      </div>
    );
  }
  ...
  const cards = [
    { id: "total", label: "Tổng bình luận", count: stats.total, clickable: false },
