...
function parseIncludeChildren(url) {
  const param = url.searchParams.get("include_children");
...
export const commerceCategoryProductsHandlers = [
  http.get("*/commerce/api/v1/categories/:categoryId/products", async ({ params, request }) => {
    await delay(400);
    ...
    if (!category || !category.is_active) {
      return HttpResponse.json(
        apiError("COMMERCE-404-CATEGORY", "Danh muc khong ton tai hoac da ngung hoat dong."),
        { status: 404 }
      );
    }
    ...
    const includeChildren = parseIncludeChildren(url);
    const filtered = filterProductsByCategory(
      mockCommerceProducts,
      categoryId,
      includeChildren
    );
    ...
    return HttpResponse.json(
      apiSuccess(200, "Lay san pham theo danh muc thanh cong.", data),
      { status: 200 }
    );
  }),
];
