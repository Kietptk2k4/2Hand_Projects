...
@Component
@Order(45)
public class UserRestrictedNotificationEventHandler implements NotificationEventHandler {

    private static final String USER_RESTRICTED = "USER_RESTRICTED";

    private final UserRestrictedNotificationPayloadParser payloadParser;
    ...
    public UserRestrictedNotificationEventHandler(
            UserRestrictedNotificationPayloadParser payloadParser,
    ...
    public boolean supports(String eventType) {
        return USER_RESTRICTED.equals(eventType);
    }

    @Override
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        UserRestrictedNotificationContext context;
        try {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.targetUserId(), USER_RESTRICTED)
            );
