...
@Component
public class PaymentSuccessNotificationPayloadParser {
    ...
    public PaymentSuccessNotificationPayloadParser(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public PaymentSuccessNotificationContext parse(NotificationEvent event) {
        ...
        if (buyerId == null) {
            throw new IllegalArgumentException("buyer_id is required for PAYMENT_SUCCESS notification event.");
        }
        ...
        if ((paymentId == null || paymentId.isBlank()) && (orderId == null || orderId.isBlank())) {
            throw new IllegalArgumentException(
                    "payment_id or order_id is required for PAYMENT_SUCCESS notification event."
            );
        }
        ...
        String amountSummary = firstNonBlank(
                textField(payload, "amount"),
                textField(payload, "final_amount"),
                textField(payload, "total_amount")
        );

        return new PaymentSuccessNotificationContext(
                buyerId,
    ...
    private JsonNode parsePayload(String rawPayload) {
        ...
        try {
            ...
            throw new IllegalArgumentException("PAYMENT_SUCCESS event payload must be valid JSON.");
        }
    }
