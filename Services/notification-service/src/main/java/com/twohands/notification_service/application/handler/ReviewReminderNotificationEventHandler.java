...
import com.twohands.notification_service.domain.commerce.ReviewReminderEligibilityPolicy;
...
@Component
@Order(39)
public class ReviewReminderNotificationEventHandler implements NotificationEventHandler {

    private static final String REVIEW_REMINDER = "REVIEW_REMINDER";
    private static final Set<String> SUPPORTED_EVENT_TYPES = Set.of(REVIEW_REMINDER, "COMMERCE_REVIEW_REMINDER");

    private final NotificationEventTypeAliasResolver eventTypeAliasResolver;
    private final ReviewReminderNotificationPayloadParser payloadParser;
    ...
    public ReviewReminderNotificationEventHandler(
            NotificationEventTypeAliasResolver eventTypeAliasResolver,
            ReviewReminderNotificationPayloadParser payloadParser,
    ...
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        if (!REVIEW_REMINDER.equals(canonicalEventType(event.eventType()))) {
            return NotificationEventHandlerResult.failure(
                    "Unsupported review reminder event type",
                    NotificationFailurePolicy.PERMANENT
            );
        }

        ReviewReminderNotificationContext context;
        try {
        ...
        if (ReviewReminderEligibilityPolicy.shouldSkip(context)) {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.buyerId(), REVIEW_REMINDER)
            );
