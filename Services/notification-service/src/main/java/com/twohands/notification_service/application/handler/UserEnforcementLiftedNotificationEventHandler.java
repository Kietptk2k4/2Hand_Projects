...
@Component
@Order(45)
public class UserEnforcementLiftedNotificationEventHandler implements NotificationEventHandler {
    ...
    private final UserEnforcementLiftedNotificationPayloadParser payloadParser;
    ...
    public UserEnforcementLiftedNotificationEventHandler(
            UserEnforcementLiftedNotificationPayloadParser payloadParser,
    ...
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        UserEnforcementLiftedNotificationContext context;
        try {
            context = payloadParser.parse(event, event.eventType());
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.targetUserId(), event.eventType())
            );
