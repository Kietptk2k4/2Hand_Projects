...
@Component
@Order(29)
public class PostLikedNotificationEventHandler implements NotificationEventHandler {

    private static final String POST_LIKED = "POST_LIKED";
    private static final String REFERENCE_TYPE = "POST";

    private final PostLikedNotificationPayloadParser payloadParser;
    ...
    public PostLikedNotificationEventHandler(
            PostLikedNotificationPayloadParser payloadParser,
    ...
    public boolean supports(String eventType) {
        return POST_LIKED.equals(eventType);
    }

    @Override
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        PostLikedNotificationContext context;
        try {
        ...
        if (context.actorId() == null) {
            return NotificationEventHandlerResult.failure(
                    "Actor id is required for POST_LIKED notification event",
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.postAuthorId(), POST_LIKED)
            );
