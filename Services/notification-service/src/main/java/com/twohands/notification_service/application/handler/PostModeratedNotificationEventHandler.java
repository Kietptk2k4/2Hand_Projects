...
@Component
@Order(44)
public class PostModeratedNotificationEventHandler implements NotificationEventHandler {

    private static final String POST_MODERATED = "POST_MODERATED";

    private final PostModeratedNotificationPayloadParser payloadParser;
    ...
    public PostModeratedNotificationEventHandler(
            PostModeratedNotificationPayloadParser payloadParser,
    ...
    public boolean supports(String eventType) {
        return POST_MODERATED.equals(eventType);
    }

    @Override
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        PostModeratedNotificationContext context;
        try {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.authorUserId(), POST_MODERATED)
            );
