...
@Component
@Order(37)
public class PaymentRefundedNotificationEventHandler implements NotificationEventHandler {
    ...
    private final PaymentRefundedNotificationPayloadParser payloadParser;
    ...
    public PaymentRefundedNotificationEventHandler(
            NotificationEventTypeAliasResolver eventTypeAliasResolver,
            PaymentRefundedNotificationPayloadParser payloadParser,
    ...
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        if (!PAYMENT_REFUNDED.equals(canonicalEventType(event.eventType()))) {
            return NotificationEventHandlerResult.failure(
                    "Unsupported payment refunded event type",
                    NotificationFailurePolicy.PERMANENT
            );
        }

        PaymentRefundedNotificationContext context;
        try {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.buyerId(), PAYMENT_REFUNDED)
            );
        ...
        String referenceId = context.paymentId() != null ? context.paymentId() : context.orderId();
