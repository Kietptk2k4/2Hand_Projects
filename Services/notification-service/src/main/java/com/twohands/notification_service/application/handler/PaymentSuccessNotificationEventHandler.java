...
@Component
@Order(36)
public class PaymentSuccessNotificationEventHandler implements NotificationEventHandler {
    ...
    public PaymentSuccessNotificationEventHandler(
    ...
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        if (!PAYMENT_SUCCESS.equals(canonicalEventType(event.eventType()))) {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.buyerId(), PAYMENT_SUCCESS)
            );
