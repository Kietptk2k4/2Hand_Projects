...
@Component
public class PayoutRequestApprovedNotificationPayloadParser {

    private static final String PAYOUT_REQUEST_AGGREGATE_TYPE = "PAYOUT_REQUEST";

    private final ObjectMapper objectMapper;

    public PayoutRequestApprovedNotificationPayloadParser(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public PayoutRequestApprovedNotificationContext parse(NotificationEvent event) {
        ...
        if (sellerId == null) {
            throw new IllegalArgumentException("seller_id is required for PAYOUT_REQUEST_APPROVED notification event.");
        }
        ...
        if (payoutRequestId == null || payoutRequestId.isBlank()) {
            throw new IllegalArgumentException("payout_request_id is required for PAYOUT_REQUEST_APPROVED notification event.");
        }

        BigDecimal amount = parseAmount(payload.get("amount"));

        return new PayoutRequestApprovedNotificationContext(
                sellerId,
                payoutRequestId.trim(),
                amount,
                textField(payload, "approved_at")
        );
    }
    ...
    private JsonNode parsePayload(String rawPayload) {
        ...
        try {
            ...
            throw new IllegalArgumentException("PAYOUT_REQUEST_APPROVED event payload must be valid JSON.");
        }
    }
