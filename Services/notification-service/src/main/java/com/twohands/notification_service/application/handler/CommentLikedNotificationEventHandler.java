...
@Component
@Order(33)
public class CommentLikedNotificationEventHandler implements NotificationEventHandler {

    private static final String COMMENT_LIKED = "COMMENT_LIKED";
    private static final String REFERENCE_TYPE = "COMMENT";

    private final CommentLikedNotificationPayloadParser payloadParser;
    ...
    public CommentLikedNotificationEventHandler(
            CommentLikedNotificationPayloadParser payloadParser,
    ...
    public boolean supports(String eventType) {
        return COMMENT_LIKED.equals(eventType);
    }

    @Override
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        CommentLikedNotificationContext context;
        try {
        ...
        if (context.actorId() == null) {
            return NotificationEventHandlerResult.failure(
                    "Actor id is required for COMMENT_LIKED notification event",
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.commentAuthorId(), COMMENT_LIKED)
            );
