...
@Component
@Order(32)
public class CommentRepliedNotificationEventHandler implements NotificationEventHandler {

    private static final String COMMENT_REPLIED = "COMMENT_REPLIED";
    private static final String REFERENCE_TYPE = "COMMENT";

    private final CommentRepliedNotificationPayloadParser payloadParser;
    ...
    public CommentRepliedNotificationEventHandler(
            CommentRepliedNotificationPayloadParser payloadParser,
    ...
    public boolean supports(String eventType) {
        return COMMENT_REPLIED.equals(eventType);
    }

    @Override
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        CommentRepliedNotificationContext context;
        try {
        ...
        if (context.actorId() == null) {
            return NotificationEventHandlerResult.failure(
                    "Actor id is required for COMMENT_REPLIED notification event",
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.parentCommentAuthorId(), COMMENT_REPLIED)
            );
