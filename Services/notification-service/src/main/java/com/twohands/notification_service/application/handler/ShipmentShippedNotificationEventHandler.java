...
@Component
@Order(40)
public class ShipmentShippedNotificationEventHandler implements NotificationEventHandler {

    private static final String SHIPMENT_SHIPPED = "SHIPMENT_SHIPPED";
    private static final String REFERENCE_TYPE = "SHIPMENT";
    private static final Set<String> SUPPORTED_EVENT_TYPES = Set.of(SHIPMENT_SHIPPED, "COMMERCE_SHIPMENT_SHIPPED");

    private final NotificationEventTypeAliasResolver eventTypeAliasResolver;
    private final ShipmentShippedNotificationPayloadParser payloadParser;
    ...
    public ShipmentShippedNotificationEventHandler(
            NotificationEventTypeAliasResolver eventTypeAliasResolver,
            ShipmentShippedNotificationPayloadParser payloadParser,
    ...
    public NotificationEventHandlerResult handle(NotificationEvent event) {
        if (!SHIPMENT_SHIPPED.equals(canonicalEventType(event.eventType()))) {
            return NotificationEventHandlerResult.failure(
                    "Unsupported shipment shipped event type",
                    NotificationFailurePolicy.PERMANENT
            );
        }

        ShipmentShippedNotificationContext context;
        try {
        ...
        try {
            deliveryDecision = applyNotificationDeliveryRulesUseCase.execute(
                    new ApplyNotificationDeliveryRulesCommand(context.buyerId(), SHIPMENT_SHIPPED)
            );
