...
@Component
public class CommentCreatedNotificationPayloadParser {
    ...
    public CommentCreatedNotificationPayloadParser(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public CommentCreatedNotificationContext parse(NotificationEvent event) {
        ...
        if (postAuthorId == null) {
            throw new IllegalArgumentException("post_author_id is required for COMMENT_CREATED notification event.");
        }
        if (postId == null || postId.isBlank()) {
            throw new IllegalArgumentException("post_id is required for COMMENT_CREATED notification event.");
        }
        if (commentId == null || commentId.isBlank()) {
            throw new IllegalArgumentException("comment_id is required for COMMENT_CREATED notification event.");
        }

        return new CommentCreatedNotificationContext(
                actorId,
                postAuthorId,
                postId.trim(),
                commentId.trim()
    ...
    private JsonNode parsePayload(String rawPayload) {
        ...
        try {
            ...
            throw new IllegalArgumentException("COMMENT_CREATED event payload must be valid JSON.");
        }
    }
