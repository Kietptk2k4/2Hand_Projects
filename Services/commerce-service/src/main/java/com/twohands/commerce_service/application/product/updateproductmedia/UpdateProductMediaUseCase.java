...
public class UpdateProductMediaUseCase {
    ...
    public UpdateProductMediaResult execute(UpdateProductMediaCommand command) {
        ...
        productMediaUrlValidator.validateProductMediaComposition(normalizedUrls);
        ...
        for (int index = 0; index < normalizedUrls.size(); index++) {
            String mediaUrl = normalizedUrls.get(index);
            ProductMediaType mediaType = productMediaContentValidator.inferMediaTypeFromUrl(mediaUrl);
            mediaItems.add(new ProductMediaItem(mediaUrl, mediaType.name(), index));
        }
        ...
        String thumbnailUrl = saved.stream()
                .filter(item -> ProductMediaType.IMAGE.name().equals(item.mediaType()))
                .map(ProductMediaItem::mediaUrl)
                .findFirst()
                .orElse(null);
    ...
    public String successMessage() {
        return "Cap nhat media san pham thanh cong.";
    }
