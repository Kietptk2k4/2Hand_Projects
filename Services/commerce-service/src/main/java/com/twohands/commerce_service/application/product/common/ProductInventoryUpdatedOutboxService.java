...
@Component
public class ProductInventoryUpdatedOutboxService {

    public static final String EVENT_TYPE = "COMMERCE_PRODUCT_INVENTORY_UPDATED";
    private static final String SOURCE = "commerce";

    private final ObjectMapper objectMapper;

    public ProductInventoryUpdatedOutboxService(ObjectMapper objectMapper) {
    ...
    public OutboxEvent build(
        ...
        payload.put("low_stock_threshold", lowStockThreshold);
        payload.put("reserved_quantity", reservedQuantity);
        payload.put("updated_at", updatedAt.toString());

        return new OutboxEvent(
                UUID.randomUUID(),
                EVENT_TYPE,
                "product:" + productId + ":inventory:updated:" + updatedAt.toEpochMilli(),
                productId,
                SOURCE,
                serialize(payload),
