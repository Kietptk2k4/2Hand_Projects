...
@Service
public class PublishCommerceEventsUseCase {

    private static final Logger log = LoggerFactory.getLogger(PublishCommerceEventsUseCase.class);
    ...
    public PublishCommerceEventsUseCase(
            OutboxEventRepository outboxEventRepository,
            OutboxEventPublisher outboxEventPublisher,
            @Value("${commerce.outbox.publish.max-retries:5}") int maxRetries,
            @Value("${commerce.outbox.publish.batch-size:50}") int batchSize
    ) {
    ...
    private void processSingleEvent(OutboxEvent event, Instant now) {
        try {
            ...
            log.info(
                    "Outbox publish success. outboxEventId={}, eventType={}, eventKey={}, aggregateId={}, newStatus=PUBLISHED",
                    event.id(),
                    event.eventType(),
                    event.eventKey(),
                    event.aggregateId()
            ...
            log.warn(
                    "Outbox publish failed. outboxEventId={}, eventType={}, eventKey={}, aggregateId={}, retryCount={}, newStatus=FAILED, error={}",
                    event.id(),
                    event.eventType(),
                    event.eventKey(),
                    event.aggregateId(),
