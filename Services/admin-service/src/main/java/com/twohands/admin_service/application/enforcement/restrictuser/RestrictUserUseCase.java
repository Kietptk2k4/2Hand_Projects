...
public class RestrictUserUseCase {
	...
	private static final Logger log = LoggerFactory.getLogger(RestrictUserUseCase.class);
	private static final String ACTION_TYPE = "USER_RESTRICT";
	private static final String OUTBOX_EVENT_TYPE = "USER_RESTRICTED";
	private static final String SUCCESS_MESSAGE = "User restricted successfully";
	...
	@Transactional
	public RestrictUserResult execute(RestrictUserCommand command) {
		UUID adminId = adminAuthorizationService.requireCurrentAdminId();
		adminAuthorizationService.requirePermission(AdminPermission.USER_RESTRICT);

		Instant now = Instant.now();
		UserEnforcementPolicy.validateRestrictRequest(
				command.reasonCode(),
				command.description(),
				command.expiresAt(),
				now
		...
		if (userEnforcementRepository.existsActiveByUserIdAndActionType(
				command.userId(),
		...
		log.info("Restricting user. adminId={}, userId={}, enforcementId={}", adminId, command.userId(), enforcementId);

		try {
			if (authUserEnforcementGateway.isEnabled()) {
				authUserEnforcementGateway.restrictUser(new AuthUserEnforcementGateway.AuthRestrictUserRequest(
						command.userId(),
						enforcementId,
			...
			OutboxEvent outboxEvent = insertAdminOutboxEventUseCase.execute(new InsertAdminOutboxEventCommand(
					OUTBOX_EVENT_TYPE,
					command.userId(),
					outboxPayloadBuilder.buildUserRestrictedPayload(enforcement)
			));
			...
			adminActionAuditLogger.logCritical(
					...
					SUCCESS_MESSAGE,
					"User restrict enforcement created",
					Map.of("status", "ACTIVE", "action_type", "RESTRICT"),
					afterSummary,
					Map.of(
