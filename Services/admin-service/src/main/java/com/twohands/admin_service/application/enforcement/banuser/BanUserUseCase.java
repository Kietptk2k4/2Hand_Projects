...
public class BanUserUseCase {
	...
	private static final String ACTION_TYPE = "USER_BAN";
	private static final String OUTBOX_EVENT_TYPE = "USER_BANNED";
	private static final String SUCCESS_MESSAGE = "User banned successfully";
	...
	@Transactional
	public BanUserResult execute(BanUserCommand command) {
		UUID adminId = adminAuthorizationService.requireCurrentAdminId();
		adminAuthorizationService.requireAnyPermission(AdminPermission.USER_BAN, AdminPermission.USER_SUSPEND);

		Instant now = Instant.now();
		UserEnforcementPolicy.validateBanRequest(command.reasonCode(), command.description(), command.expiresAt(), now);

		if (userEnforcementRepository.existsActiveByUserIdAndActionType(command.userId(), UserEnforcementActionType.BAN)) {
		...
		log.info("Banning user. adminId={}, userId={}, enforcementId={}", adminId, command.userId(), enforcementId);

		try {
			if (authUserEnforcementGateway.isEnabled()) {
				authUserEnforcementGateway.banUser(new AuthUserEnforcementGateway.AuthBanUserRequest(
						command.userId(),
						enforcementId,
			...
			OutboxEvent outboxEvent = insertAdminOutboxEventUseCase.execute(new InsertAdminOutboxEventCommand(
					OUTBOX_EVENT_TYPE,
					command.userId(),
					outboxPayloadBuilder.buildUserBannedPayload(enforcement)
			));
			...
			adminActionAuditLogger.logCritical(
					...
					SUCCESS_MESSAGE,
					"User ban enforcement created",
					Map.of("status", "ACTIVE", "action_type", "BAN"),
					afterSummary,
					Map.of(
