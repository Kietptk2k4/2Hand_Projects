...
class ViewPostDetailApiIntegrationTest {
    ...
    void shouldReturnUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/social/posts/507f1f77bcf86cd799439011")
    ...
    @Test
    void shouldReturnPostDetailWhenAuthenticated() throws Exception {
        UUID viewerId = UUID.randomUUID();
        UUID authorId = UUID.randomUUID();
        String postId = "507f1f77bcf86cd799439011";
        String token = buildAccessToken(viewerId);

        ViewPostDetailResult result = new ViewPostDetailResult(
                postId,
        ...
        when(viewPostDetailUseCase.execute(eq(viewerId), eq(postId))).thenReturn(result);
        when(viewPostDetailUseCase.successMessage()).thenReturn("Lay chi tiet bai viet thanh cong.");
        ...
        mockMvc.perform(get("/api/v1/social/posts/{postId}", postId)
                ...
                .andExpect(jsonPath("$.message").value("Lay chi tiet bai viet thanh cong."))
                .andExpect(jsonPath("$.data.postId").value(postId))
                .andExpect(jsonPath("$.data.author.userId").value(authorId.toString()))
                .andExpect(jsonPath("$.data.author.displayName").value("User A"))
                .andExpect(jsonPath("$.data.likedByMe").value(true))
                .andExpect(jsonPath("$.data.savedByMe").value(false))
                .andExpect(jsonPath("$.data.isOwner").value(false))
                .andExpect(jsonPath("$.data.productTags[0].productId").value("product-1"));
    }
