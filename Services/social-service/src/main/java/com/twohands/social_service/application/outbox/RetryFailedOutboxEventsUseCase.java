...
public class RetryFailedOutboxEventsUseCase {
    ...
    public RetryFailedOutboxEventsUseCase(
            ...
            @Value("${social.outbox.retry.max-retries:5}") int maxRetries,
            @Value("${social.outbox.retry.pending-timeout-seconds:300}") int pendingTimeoutSeconds,
            @Value("${social.outbox.retry.batch-size:50}") int batchSize
    ) {
