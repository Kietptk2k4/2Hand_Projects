...
Provide a single HTTPS reverse-proxy gateway (nginx + one ngrok tunnel) for remote development demos so web, mobile, PayOS webhooks, and MinIO object access share one public origin with path-based routing.
...
- **WHEN** the ngrok compose profile is active and the ngrok tunnel is online
...
- **WHEN** the ngrok compose profile is not started
