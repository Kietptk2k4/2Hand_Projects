...
- Màn chi tiết post / comment sheet: gọi top-level trước; khi user mở reply gọi thêm với `parent_comment_id`.
- Dùng `meta.hasNext` cho infinite scroll.
- Response JSON dùng camelCase (`commentId`, `parentCommentId`, `totalElements`).
