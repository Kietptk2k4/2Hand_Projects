...
- **Comment media (FE):** Reply dùng `CommentComposer.jsx` inline trên `CommentItem.jsx`; upload/hiển thị cùng luồng với top-level comment (`useCommentMediaUpload.js`, `mapCommentMediaPayload.js`, `submitReply` trong `usePostComments.js`).
...
- **Field mapping:** `commentId`, `postId`, `parentCommentId`, `authorId`, `contentText`, `media`, `createdAt`, `updatedAt`.
- Tham chiếu: `docs/feature_requirements/social/FR_ReplyComment.md`, `docs/engineering_rules/frontend-api-integration.md`.
